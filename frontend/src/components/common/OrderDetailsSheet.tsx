import { Copy, Mail, Phone, Package, CreditCard, Calendar, Hash, User, Printer, Truck } from "lucide-react";
              ...
              <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
                ...
                <div className="flex items-center gap-2">
                  <Button variant="outline" size="sm" className="h-9 gap-1.5"
                    onClick={() => { void navigator.clipboard.writeText(order.id); toast.success("Order ID copied"); }}>
                ...
                <div className="lg:col-span-2 space-y-6">
                  <Section title="Line items">
                    <div className="rounded-lg border border-border overflow-hidden">
                      <div className="flex items-center justify-between p-4 bg-muted/30">
                        <div className="flex items-center gap-3 min-w-0">
                          ...
                          <div className="min-w-0">
                            ...
                            <p className="text-xs text-muted-foreground">SKU · FP-{order.id.slice(3)} · Qty 1</p>
                          </div>
                        </div>
                      ...
                      <div className="p-4 space-y-2 text-sm">
                        <SummaryRow label="Subtotal" value={`$${order.amount.toFixed(2)}`} />
                        <SummaryRow label="Shipping" value="$0.00" muted />
                        <SummaryRow label="Tax" value="$0.00" muted />
                        <Separator className="my-2" />
                        <SummaryRow label="Total" value={`$${order.amount.toFixed(2)}`} bold />
                      </div>
                    </div>
                  </Section>
                  ...
                  <Section title="Timeline">
                    <ol className="relative border-l border-border ml-2 space-y-4">
                      {[
                        { label: "Order placed", time: order.createdAt, active: true },
                        { label: "Payment " + prettyPayment(order.payment).toLowerCase(), time: order.createdAt, active: order.payment === "PAID" },
                        { label: prettyStatus(order.status), time: order.createdAt, active: true },
                      ].map((e, i) => (
                        <li key={i} className="ml-4">
                          <span className={`absolute -left-[5px] h-2.5 w-2.5 rounded-full ring-4 ring-background ${e.active ? "bg-primary" : "bg-muted-foreground/40"}`} />
                          <p className="text-sm font-medium text-foreground">{e.label}</p>
                          <p className="text-xs text-muted-foreground">{formatFull(e.time)}</p>
                        </li>
                      ))}
                    </ol>
                  </Section>
                ...
                <div className="space-y-6">
                  <Section title="Customer">
                    <div className="rounded-lg border border-border p-4 space-y-3">
                      <InfoRow icon={User} label="Name" value={order.customer} />
                      <InfoRow icon={Mail} label="Email" value={order.email}
                        action={() => { void navigator.clipboard.writeText(order.email); toast.success("Email copied"); }} />
                      <InfoRow icon={Phone} label="Phone" value={order.phone}
                        action={() => { void navigator.clipboard.writeText(order.phone); toast.success("Phone copied"); }} />
                    </div>
                  </Section>
                  ...
                  <Section title="Payment">
                    <div className="rounded-lg border border-border p-4 space-y-3">
                      ...
                      <InfoRow icon={Hash} label="Reference" value={`TXN-${order.id.slice(3)}`} />
                      <InfoRow icon={Calendar} label="Date" value={formatFull(order.createdAt)} />
                    </div>
                  </Section>
                </div>
              </div>
